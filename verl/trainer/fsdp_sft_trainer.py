...
# Make logger actually output to console in rank-0
if not logger.handlers:
    _h = logging.StreamHandler()
    _h.setFormatter(logging.Formatter('[%(asctime)s][%(levelname)s] %(message)s', '%H:%M:%S'))
    logger.addHandler(_h)
# Honor env var, default INFO instead of WARN
...
class FSDPSFTTrainer(object):
    ...
    def fit(self):
        ...
        log_every = int(getattr(self.config.trainer, 'log_every_n_steps', 10))
        for epoch in range(self.config.trainer.total_epochs):
            self.train_sampler.set_epoch(epoch=epoch)
            iterable = self.train_dataloader
            if rank == 0:
                iterable = tqdm(iterable, total=len(self.train_dataloader), desc=f"Epoch {epoch}", leave=False)

            for data in iterable:
                data = TensorDict(data, batch_size=self.config.data.train_batch_size).cuda()
                metric = self.training_step(data)
                if rank == 0:
                    # Tracking backends (wandb/console)
                    tracking.log(data=metric, step=global_step)
                    if (global_step % log_every) == 0:
                        # Pretty console line
                        lr = self.lr_scheduler.get_last_lr()[0]
                        logger.info(f"step={global_step:>6} epoch={epoch} loss={metric['train/loss']:.4f} lr={lr:.6e}")
                global_step += 1
                ...
                if global_step >= self.total_training_steps:
                    ...
                    for val_data in self.val_dataloader:
                        ...
                        if rank == 0:
                            logger.info(f"[val] epoch={epoch} step={global_step} val_loss={val_loss:.4f}")
                        val_losses.append(val_loss)
                    if rank == 0:
