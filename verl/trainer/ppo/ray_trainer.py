...
import re
from chat_r1.llm_agent.generation import LLMGenerationManager, GenerationConfig
...
class RayPPOTrainer(object):
    ...
    def _validate(self):
        ...
        reward_tensor_lst = []
        data_source_lst = []
        results = []  # To store results for saving in JSON if config.save_val is True
        save_every_k_step = 1000000
        step_count = 0
        ...
        if not self.config.do_search:
            for test_data in self.val_dataloader:
                ...
                # Save results
                # if self.config.save_val and step_count % save_every_k_step == 0:
                #         # print("save here")
                #         # print("-------------------------------")
                #         # print("-------------------------------")
                #         # print("-------------------------------")
                #         os.makedirs(self.config.save_val_dir, exist_ok=True)
                #         save_path = os.path.join(self.config.save_val_dir, f'validation_results_step{step_count}.json')
                ...
                if self.config.save_val:
                    for i in range(len(test_batch.batch['responses'])):
                        # try:
                        # Get input IDs (tensor), convert to list, then decode
                        response_ids = test_batch.batch['responses'][i]
                        attention_mask = test_batch.batch['attention_mask'][i]

                        # Assuming attention_mask includes both prompt + response,
                        # and response starts right after the prompt in the same way as in __call__
                        prompt_len = test_batch.batch['prompts'][i].shape[0]
                        valid_response_len = attention_mask[prompt_len:].sum().item()
                        valid_response_ids = response_ids[:valid_response_len]

                        # print(valid_response_ids)
                        # print(self.tokenizer.decode(valid_response_ids, skip_special_tokens=True))

                        # Decode predicted answer
                        raw_predicted_answer = self.tokenizer.decode(valid_response_ids, skip_special_tokens=True)
                        predicted_answer = qa_em.extract_solution(raw_predicted_answer)
            ...
            for batch_dict in self.val_dataloader:
                ...
                if self.config.save_val:
                    for i in range(len(test_batch.batch['responses'])):
                        ...
                        #         "question": test_batch.non_tensor_batch['question'][i],
                        #         "golden_answers": test_batch.non_tensor_batch['golden_answers'][i],
                        #         "predicted_answer": test_batch.batch['responses'][i].tolist()
                        #     })
                        
                    # # Save intermediate results every k steps
                    # if self.config.save_val and step_count % save_every_k_step == 0:
                    #     # print("save here")
                    #     # print("-------------------------------")
                    #     # print("-------------------------------")
                    ...
                    #         json.dump(results, f, indent=4)
                    #     results=[]
        # Save last results, because last are not saved in the loop
        if self.config.save_val:
            os.makedirs(self.config.save_val_dir, exist_ok=True)
            ...
            results=[]
                ...
                #     with open(os.path.join(self.config.save_val_dir, 'validation_results.json'), 'w') as f:
                #         json.dump(results, f, indent=4)
