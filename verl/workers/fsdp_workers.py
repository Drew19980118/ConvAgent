...
class ActorRolloutRefWorker(Worker):
    ...
    def generate_sequences(self, prompts: DataProto):
        ...
        if prompts is None or len(prompts.batch['input_ids']) == 0:
            print("Empty batch detected in generate_sequences. Returning empty output.")
            return DataProto(batch={}, non_tensor_batch={}, meta_info={})  # Return an empty DataProto
        ...
        if self._is_actor and recompute_log_prob:
            ...
            # Compute current per-rank batch size safely
            _cur_bs = output.select(batch_keys=['input_ids']).batch['input_ids'].size(0)  # here
            _cfg_mb = int(self.config.rollout.log_prob_micro_batch_size)                 # here
            _eff_mb = max(1, min(_cfg_mb, _cur_bs))                                      # here
            output.meta_info['micro_batch_size'] = _eff_mb                               # here
    ...
    def compute_ref_log_prob(self, data: DataProto):
        ...
        # Determine current per-rank batch size from the ref batch you’re about to use
        _ref_bs = data.select(batch_keys=['input_ids']).batch['input_ids'].size(0)  # here
        _cfg_ref_mb = int(self.config.ref.log_prob_micro_batch_size)                    # here
        _ref_eff_mb = max(1, min(_cfg_ref_mb, _ref_bs))                                 # here
        micro_batch_size = _ref_eff_mb                                                  # here
