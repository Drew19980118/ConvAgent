...
class ActorRolloutRefWorker(MegatronWorker):
    ...
    def generate_sequences(self, prompts: DataProto):
        ...
        if self._is_actor and not validate:
            ...
            _m_bs = output.select(batch_keys=['input_ids']).batch['input_ids'].size(0)  # here
            _cfg_mb = int(self.config.rollout.log_prob_micro_batch_size)                # here
            _eff_mb = max(1, min(_cfg_mb, _m_bs))                                       # here
    ...
    def compute_ref_log_prob(self, data: DataProto):
        ...
        _bs = data.select(batch_keys=['input_ids']).batch['input_ids'].size(0)  # here
        _cfg = int(self.config.rollout.log_prob_micro_batch_size)                     # here
        micro_batch_size = max(1, min(_cfg, _bs))                                     # here
