...
class LLMGenerationManager:
    ...
    def _generate_with_gpu_padding(self, active_batch: DataProto) -> DataProto:
        ...
        if batch_size == 0:
            print("Batch size is 0, returning empty DataProto.")
            # Return empty DataProto
            empty_batch = {k: torch.empty((0,) + v.shape[1:], dtype=v.dtype, device=v.device)
                        for k, v in active_batch.batch.items()}
            return DataProto.from_dict(empty_batch)
        ...
        print("[INFO] Padding active_batch with {} sequences".format(padding_size))
        padded_batch = {}
        
        for k, v in active_batch.batch.items():
            pad_value = v[0:1]
            if k == "attention_mask":
                # Ensure padded rows are valid (not dropped during unpadding)
                pad_sequence = torch.ones_like(pad_value).repeat(padding_size, *[1] * (v.ndim - 1))
            else:
                pad_sequence = pad_value.repeat(padding_size, *[1] * (v.ndim - 1))
        ...
        try:
            ...
            print("Length of active_batch:", len(active_batch.batch['input_ids']))
            print("Length of padded_active_batch:", len(padded_active_batch.batch['input_ids']))
            raise e
