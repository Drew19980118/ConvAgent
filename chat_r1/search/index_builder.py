...
class Index_Builder:
    ...
    def encode_all(self):
        ...
        for start_idx in tqdm(range(0, len(self.corpus), self.batch_size), desc='Inference Embeddings:'):
            if "title" in self.corpus[start_idx:start_idx+self.batch_size]:
                ...
                if 'contents' in self.corpus[start_idx:start_idx+self.batch_size]:
                    batch_data_text = self.corpus[start_idx:start_idx+self.batch_size]['contents']
                    batch_data = [text for text in batch_data_text]
                elif 'text' in self.corpus[start_idx:start_idx+self.batch_size]:
                    batch_data_text = self.corpus[start_idx:start_idx+self.batch_size]['text']
                    batch_data = [text for text in batch_data_text]
